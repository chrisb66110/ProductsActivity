...
namespace ProductsActivity.Bll.Test.BllsTest.GenderBllTest
{
    ...
    [TestClass]
    public class GetLogsAsyncTest : BaseTest<GenderBll>
    {
        [TestMethod]
        public async Task GetLogsAsyncHappyPath()
        {
            ...
            {
                var responseRepository = DataTestHelper.GivenTheDefaultListGenderLogDto();

                AndIMockDependencyMethod<IGenderRepository, List<GenderLogDto>>(autoMock, m => m.GetLogsAsync(), responseRepository);
        ...
        public async Task GetLogsAsyncErrorPath()
        {
            using (var autoMock = AutoMock.GetStrict())
            {
                ...
                AndIMockDependencyMethod<IGenderRepository, List<GenderLogDto>, Exception>(autoMock, m => m.GetLogsAsync(), responseRepository);
