...
namespace ProductsActivity.Api.Test.ControllersTest.ProductLikeControllerTest
{
    ...
    [TestClass]
    public class UpdateAsyncTest : BaseTest<ProductLikeController>
    {
        [TestMethod]
        public async Task UpdateAsyncHappyPath()
        {
            using (var autoMock = AutoMock.GetStrict(RegisterBasicDependency))
            {
                ...
                AndIMockDependencyMethod<IProductLikeBll, ProductLikeDto, ProductLikeDto>(autoMock, m => m.UpdateAsync(It.IsAny<ProductLikeDto>()), responseBll, param =>
                {
        ...
        public async Task UpdateAsyncConflictPath()
        {
            using (var autoMock = AutoMock.GetStrict(RegisterBasicDependency))
            {
                ...
                AndIMockDependencyMethod<IProductLikeBll, ProductLikeDto, ProductLikeDto>(autoMock, m => m.UpdateAsync(It.IsAny<ProductLikeDto>()), null, param =>
                {
                    CheckAllProperties(request, param);
                });
        ...
        public async Task UpdateAsyncErrorPath()
        {
            using (var autoMock = AutoMock.GetStrict(RegisterBasicDependency))
            {
                ...
                AndIMockDependencyMethod<IProductLikeBll, ProductLikeDto, Exception>(autoMock, m => m.UpdateAsync(It.IsAny<ProductLikeDto>()), exception);
