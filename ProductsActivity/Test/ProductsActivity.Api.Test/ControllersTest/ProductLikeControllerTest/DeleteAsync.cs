...
namespace ProductsActivity.Api.Test.ControllersTest.ProductLikeControllerTest
{
    ...
    [TestClass]
    public class DeleteAsyncTest : BaseTest<ProductLikeController>
    {
        [TestMethod]
        public async Task DeleteAsyncHappyPath()
        {
            using (var autoMock = AutoMock.GetStrict(RegisterBasicDependency))
            {
                ...
                AndIMockDependencyMethod<IProductLikeBll, ProductLikeDto, ProductLikeDto>(autoMock, m => m.DeleteAsync(It.IsAny<ProductLikeDto>()), responseBll, param =>
                {
        ...
        public async Task DeleteAsyncConflictPath()
        {
            using (var autoMock = AutoMock.GetStrict(RegisterBasicDependency))
            {
                ...
                AndIMockDependencyMethod<IProductLikeBll, ProductLikeDto, ProductLikeDto>(autoMock, m => m.DeleteAsync(It.IsAny<ProductLikeDto>()), null, param =>
                {
                    CheckAllProperties(request, param);
                });
        ...
        public async Task DeleteAsyncErrorPath()
        {
            using (var autoMock = AutoMock.GetStrict(RegisterBasicDependency))
            {
                ...
                AndIMockDependencyMethod<IProductLikeBll, ProductLikeDto, Exception>(autoMock, m => m.DeleteAsync(It.IsAny<ProductLikeDto>()), exception);
