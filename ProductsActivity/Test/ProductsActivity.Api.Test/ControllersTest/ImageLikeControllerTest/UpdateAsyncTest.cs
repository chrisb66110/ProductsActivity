...
namespace ProductsActivity.Api.Test.ControllersTest.ImageLikeControllerTest
{
    ...
    [TestClass]
    public class UpdateAsyncTest : BaseTest<ImageLikeController>
    {
        [TestMethod]
        public async Task UpdateAsyncHappyPath()
        {
            using (var autoMock = AutoMock.GetStrict(RegisterBasicDependency))
            {
                ...
                AndIMockDependencyMethod<IImageLikeBll, ImageLikeDto, ImageLikeDto>(autoMock, m => m.UpdateAsync(It.IsAny<ImageLikeDto>()), responseBll, param =>
                {
        ...
        public async Task UpdateAsyncConflictPath()
        {
            using (var autoMock = AutoMock.GetStrict(RegisterBasicDependency))
            {
                ...
                AndIMockDependencyMethod<IImageLikeBll, ImageLikeDto, ImageLikeDto>(autoMock, m => m.UpdateAsync(It.IsAny<ImageLikeDto>()), null, param =>
                {
                    CheckAllProperties(request, param);
                });
        ...
        public async Task UpdateAsyncErrorPath()
        {
            using (var autoMock = AutoMock.GetStrict(RegisterBasicDependency))
            {
                ...
                AndIMockDependencyMethod<IImageLikeBll, ImageLikeDto, Exception>(autoMock, m => m.UpdateAsync(It.IsAny<ImageLikeDto>()), exception);
