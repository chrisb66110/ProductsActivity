...
namespace ProductsActivity.Dal.Test.RepositoriesTest.CategoryRepositoryTest
{
    ...
    [TestClass]
    public class GetLogsAsyncTest : BaseRepositoryTest<CategoryRepository, ProductsActivityContext>
    {
        [TestMethod]
        public async Task GetLogsAsyncHappyPath()
        {
            ...
            {
                var entities = DataTestHelper.GivenTheDefaultListCategoryLog();
                ...
                for (int index = 0; index < entities.Count; index++)
                {
                    ...
                    {
                        var dto = JsonConvert.DeserializeObject<CategoryDto>(entities[index].PreviousValue);
                    ...
                    {
                        var dto = JsonConvert.DeserializeObject<CategoryDto>(entities[index].NewValue);
                        CheckAllProperties(dto, response[index].NewEntity);
                    }
                }

                var tableData = AndIGetTableData<CategoryLog>();
