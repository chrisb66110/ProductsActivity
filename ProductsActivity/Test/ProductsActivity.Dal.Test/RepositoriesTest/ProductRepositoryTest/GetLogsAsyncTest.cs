...
namespace ProductsActivity.Dal.Test.RepositoriesTest.ProductRepositoryTest
{
    ...
    [TestClass]
    public class GetLogsAsyncTest : BaseRepositoryTest<ProductRepository, ProductsActivityContext>
    {
        [TestMethod]
        public async Task GetLogsAsyncHappyPath()
        {
            ...
            {
                var entities = DataTestHelper.GivenTheDefaultListProductLog();
                ...
                for (int index = 0; index < entities.Count; index++)
                {
                    ...
                    {
                        var dto = JsonConvert.DeserializeObject<ProductDto>(entities[index].PreviousValue);
                    ...
                    {
                        var dto = JsonConvert.DeserializeObject<ProductDto>(entities[index].NewValue);
                        CheckAllProperties(dto, response[index].NewEntity);
                    }
                }

                var tableData = AndIGetTableData<ProductLog>();
