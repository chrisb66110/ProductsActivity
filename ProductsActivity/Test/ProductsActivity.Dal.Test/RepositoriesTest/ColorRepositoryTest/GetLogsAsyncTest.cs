...
namespace ProductsActivity.Dal.Test.RepositoriesTest.ColorRepositoryTest
{
    ...
    [TestClass]
    public class GetLogsAsyncTest : BaseRepositoryTest<ColorRepository, ProductsActivityContext>
    {
        [TestMethod]
        public async Task GetLogsAsyncHappyPath()
        {
            ...
            {
                var entities = DataTestHelper.GivenTheDefaultListColorLog();
                ...
                for (int index = 0; index < entities.Count; index++)
                {
                    ...
                    {
                        var dto = JsonConvert.DeserializeObject<ColorDto>(entities[index].PreviousValue);
                    ...
                    {
                        var dto = JsonConvert.DeserializeObject<ColorDto>(entities[index].NewValue);
                        CheckAllProperties(dto, response[index].NewEntity);
                    }
                }

                var tableData = AndIGetTableData<ColorLog>();
