...
namespace ProductsActivity.Dal.Test.RepositoriesTest.SizeRepositoryTest
{
    ...
    [TestClass]
    public class GetLogsAsyncTest : BaseRepositoryTest<SizeRepository, ProductsActivityContext>
    {
        [TestMethod]
        public async Task GetLogsAsyncHappyPath()
        {
            ...
            {
                var entities = DataTestHelper.GivenTheDefaultListSizeLog();
                ...
                for (int index = 0; index < entities.Count; index++)
                {
                    ...
                    {
                        var dto = JsonConvert.DeserializeObject<SizeDto>(entities[index].PreviousValue);
                    ...
                    {
                        var dto = JsonConvert.DeserializeObject<SizeDto>(entities[index].NewValue);
                        CheckAllProperties(dto, response[index].NewEntity);
                    }
                }

                var tableData = AndIGetTableData<SizeLog>();
