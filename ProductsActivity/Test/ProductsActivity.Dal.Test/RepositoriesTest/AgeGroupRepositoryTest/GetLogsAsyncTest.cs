...
namespace ProductsActivity.Dal.Test.RepositoriesTest.AgeGroupRepositoryTest
{
    ...
    [TestClass]
    public class GetLogsAsyncTest : BaseRepositoryTest<AgeGroupRepository, ProductsActivityContext>
    {
        [TestMethod]
        public async Task GetLogsAsyncHappyPath()
        {
            ...
            {
                var entities = DataTestHelper.GivenTheDefaultListAgeGroupLog();
                ...
                for (int index = 0; index < entities.Count; index++)
                {
                    ...
                    {
                        var dto = JsonConvert.DeserializeObject<AgeGroupDto>(entities[index].PreviousValue);
                    ...
                    {
                        var dto = JsonConvert.DeserializeObject<AgeGroupDto>(entities[index].NewValue);
                        CheckAllProperties(dto, response[index].NewEntity);
                    }
                }

                var tableData = AndIGetTableData<AgeGroupLog>();
